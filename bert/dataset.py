...
class BertTextDataset(data.Dataset):
    def __init__(self, root_dir: str, split_name: str, max_line_len: int = 150):
        ...
        :param max_line_len: biggest acceptable sequence length, longer sequences will be truncated to max_len.
    ...
    def __getitem__(self, index: int):
        ...
        encoded_input = self.tokenizer(line,
                                       ...
                                       padding='max_length',
                                       truncation=True,
                                       max_length=self.max_line_len,
                                       add_special_tokens=True,
                                       return_special_tokens_mask=True)

        line_len = min(len(labels), self.max_line_len)
        ...
        adapted_labels = np.zeros(self.max_line_len)
